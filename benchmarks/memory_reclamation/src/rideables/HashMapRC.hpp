...
class HashMapRC : public RUnorderedMap<K,V>, public RetiredMonitorable{
  struct Node;
...
private:
  ...
  const int idxSize;
  padded<marked_arc_ptr<Node>>* bucket;
  bool findNode(marked_rc_ptr<Node> &prev, marked_rc_ptr<Node> &cur, marked_rc_ptr<Node> &nxt, K key, int tid);
...
public:
  HashMapRC(GlobalTestConfig* gtc,int idx_size):
    RetiredMonitorable(gtc),idxSize(idx_size){
    bucket=new padded<marked_arc_ptr<Node>>[idxSize]{};
    int task_num = 256;
...
template <class K, class V> 
class HashMapRCFactory : public RideableFactory{
public: 
  HashMapRC<K,V>* build(GlobalTestConfig* gtc){
    // std::cout << "Prefill with " << gtc->getEnv("prefill") << std::endl;
    return new HashMapRC<K,V>(gtc,atoi((gtc->getEnv("prefill")).c_str()));
    // return new HashMapRC<K,V>(gtc,3000);
  }
};

//-------Definition----------
template <class K, class V> 
optional<V> HashMapRC<K,V>::get(K key, int tid) {
  reportAlloc(tid);
  marked_rc_ptr<Node> prev;
  marked_rc_ptr<Node> cur;
  marked_rc_ptr<Node> nxt;
  optional<V> res={};
...
template <class K, class V> 
optional<V> HashMapRC<K,V>::put(K key, V val, int tid) {
  return {};
}

template <class K, class V> 
bool HashMapRC<K,V>::insert(K key, V val, int tid){
  // int cc = 0;
  reportAlloc(tid);
  ...
  while(true){
    // cc++;
    // if(cc % 1000 == 0) std::cout << "in insert for way too long" << std::endl;
    if(findNode(prev,cur,nxt,key,tid)){
      res=false;
      break;
    }
...
template <class K, class V> 
optional<V> HashMapRC<K,V>::remove(K key, int tid) {
  // int cc = 0;
  reportAlloc(tid);
  size_t idx=hash_fn(key)%idxSize;
  marked_rc_ptr<Node> prev;
  ...
  while(true){
    // cc++;
    // if(cc % 1000 == 0) std::cout << "in remove for way too long" << std::endl;
    if(!findNode(prev,cur,nxt,key,tid)){
      res={};
      break;
    }
    res=cur->val;
    assert(nxt.get_mark() == 0);
    marked_snapshot_ptr<Node> ss = cur->next.get_snapshot();
    marked_rc_ptr<Node> ms = cur->next.load();
    // if(cc % 1000 == 0) std::cout << nxt.get_mark() << " " << ss.get_mark() << " " << ms.get_mark() << " " << nxt.get() << " " << ss.get() << std::endl;
...
template <class K, class V> 
optional<V> HashMapRC<K,V>::replace(K key, V val, int tid) {
  return {};
}

template <class K, class V> 
bool HashMapRC<K,V>::findNode(marked_rc_ptr<Node> &prev, marked_rc_ptr<Node> &cur, marked_rc_ptr<Node> &nxt, K key, int tid){
  // int cc = 0;
  while(true){
    prev.clear();
    nxt.clear();
    ...
    while(true){//to lock old and cur
      // cc++;
      // if(cc % 1000 == 0) {
      //   std::cout << "in here for way too long" << std::endl;
      // }
      if(!cur) return false;
      marked_arc_ptr<Node>* addr = (prev ? &(prev->next) : &bucket[idx].ui);
