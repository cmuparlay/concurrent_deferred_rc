...
void stress_test1() {
  atomic_shared_ptr<node> atomic_ptr(make_shared<node>(0, 0));
  ...
  for(int i = 0; i < M; i++) {
    atomic_ptr.store(make_shared<node>(i, i)); 
  }
  other1.join();
...
int main () {
  cout << "Testing Folly Atomic Shared Pointer" << endl;
  ...
  atomic_shared_ptr<node> atomic_ptr(make_shared<node>(5, 6));
  thread other ([&] () {
