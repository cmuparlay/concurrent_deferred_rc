...
inline void thread_registry_deregister_thread(const int tid);
...
inline thread_local ThreadCheckInCheckOut tl_tcico {};

// Forward declaration of global/singleton instance
class ThreadRegistry;
extern inline ThreadRegistry gThreadRegistry;
...
public:
    ...
    int register_thread_new(void) {
        for (int tid = 0; tid < REGISTRY_MAX_THREADS; tid++) {
            ...
            tl_tcico.tid = tid;
#ifdef USE_MEMBARRIER
            membarrier_cmd(MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED);
#endif
            return tid;
        }
...
inline ThreadRegistry gThreadRegistry {};
