...
#ifndef SORTED_UNORDEREDMAP_RC
#define SORTED_UNORDEREDMAP_RC
...
class SortedUnorderedMapRC : public RUnorderedMap<K,V>, public RetiredMonitorable{
  struct Node;
...
public:
  SortedUnorderedMapRC(GlobalTestConfig* gtc,int idx_size):
...
template <class K, class V> 
class SortedUnorderedMapRCFactory : public RideableFactory{
public: 
  SortedUnorderedMapRC<K,V>* build(GlobalTestConfig* gtc){
    return new SortedUnorderedMapRC<K,V>(gtc,atoi((gtc->getEnv("prefill")).c_str()));
  }
};

//-------Definition----------
template <class K, class V> 
optional<V> SortedUnorderedMapRC<K,V>::get(K key, int tid) {
  reportAlloc(tid);
...
template <class K, class V> 
optional<V> SortedUnorderedMapRC<K,V>::remove(K key, int tid) {
  reportAlloc(tid);
  size_t idx=hash_fn(key)%idxSize;
  marked_rc_ptr<Node> prev;
  marked_rc_ptr<Node> cur;
