...
// A wait-free atomic counter pair that supports increment and decrement,
// such that attempting to increment the counter from zero fails and
// does not perform the increment.
//
// Useful for implementing reference counting, where the underlying
...
public:
  ...
  StickyCounterPair() noexcept : x(1) {}
  explicit StickyCounterPair(T desired) noexcept : x(desired == 0 ? zero_flag : desired) {}
