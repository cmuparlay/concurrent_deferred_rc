...
class herlihy_arc_ptr {
  ...
  using rc_ptr_t = herlihy_rc_ptr<T, false>;
...
public:
  ...
  void store(rc_ptr_t desired) noexcept {
    ...
    // if(new_ptr != nullptr) add_to_rc(new_ptr, 1);
    while(true) {
      if(atomic_ptr.compare_exchange_strong(old_ptr, new_ptr)) {
        destroy(old_ptr);
        return;
      }
    }
...
private:
  ...
  static uint64_t add_to_rc(internal::herlihy_counted_object<T>* ptr, int v) {
    uint64_t old_rc;
    while(true) {
      old_rc = ptr->ref_cnt.load(std::memory_order_seq_cst);
      if(ptr->ref_cnt.compare_exchange_strong(old_rc, old_rc+v)) {
        return old_rc;
      }
    }
  }
  ...
  static void destroy(internal::herlihy_counted_object<T>* ptr) {
    if (ptr != nullptr && add_to_rc(ptr, -1) == 1u) {
      ptr->destroy_object();
      ar.retire(ptr);
    }
  }
